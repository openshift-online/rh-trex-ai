...
3. Start the server with auth disabled:
   ...
   make run-no-auth
   ```
   This runs `./my-service migrate` then `./my-service serve --enable-authz=false --enable-jwt=false`.

4. The server starts on `localhost:8000`. Verify it's running:
   ```bash
   curl -s http://localhost:8000/api/my-service | jq
   ```
...
```bash
curl -s http://localhost:8000/api/my-service/v1/ | jq
```
