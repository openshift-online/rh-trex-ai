...
func TestGRPCProjectCRUD(t *testing.T) {
	...
	grpcClient := pb.NewProjectServiceClient(conn)

	createReq := &pb.CreateProjectRequest{
		Name:          "TestName",
		Description:   func() *string { s := "TestDescription"; return &s }(),
		RepositoryUrl: func() *string { s := "TestRepositoryUrl"; return &s }(),
		Status:        "TestStatus",
	}
	created, err := grpcClient.CreateProject(ctx, createReq)
	Expect(err).NotTo(HaveOccurred())
	Expect(created.Metadata.Id).NotTo(BeEmpty())

	projectID := created.Metadata.Id

	getReq := &pb.GetProjectRequest{Id: projectID}
	retrieved, err := grpcClient.GetProject(ctx, getReq)
	Expect(err).NotTo(HaveOccurred())
	Expect(retrieved.Metadata.Id).To(Equal(projectID))

	updateReq := &pb.UpdateProjectRequest{
		Id:            projectID,
		Name:          func() *string { s := "UpdatedName"; return &s }(),
		Description:   func() *string { s := "UpdatedDescription"; return &s }(),
		RepositoryUrl: func() *string { s := "UpdatedRepositoryUrl"; return &s }(),
		Status:        func() *string { s := "UpdatedStatus"; return &s }(),
	}
	updated, err := grpcClient.UpdateProject(ctx, updateReq)
	Expect(err).NotTo(HaveOccurred())
	Expect(updated.Metadata.Id).To(Equal(projectID))

	listReq := &pb.ListProjectsRequest{
		Page: 1,
		Size: 10,
	}
	listResp, err := grpcClient.ListProjects(ctx, listReq)
	Expect(err).NotTo(HaveOccurred())
	Expect(listResp.Metadata.Total).To(BeNumerically(">=", 1))

	deleteReq := &pb.DeleteProjectRequest{Id: projectID}
	_, err = grpcClient.DeleteProject(ctx, deleteReq)
	Expect(err).NotTo(HaveOccurred())

	_, err = grpcClient.GetProject(ctx, getReq)
	Expect(err).To(HaveOccurred())
}

func TestGRPCWatchProjects(t *testing.T) {
	...
	go func() {
		...
		for name := range itemNames {
			...
			_, resp, postErr := client.DefaultAPI.ApiRhTrexAiV1ProjectsPost(ctx).Project(projectInput).Execute()
			if postErr != nil {
	...
	go func() {
		...
		stream, streamErr := grpcClient.WatchProjects(watchCtx, &pb.WatchProjectsRequest{})
		if streamErr != nil {
			sinkErr = fmt.Errorf("WatchProjects failed: %v", streamErr)
			close(sinkReady)
			return
		}
	...
	listResp, listErr := grpcClient.ListProjects(context.Background(), &pb.ListProjectsRequest{
		Page: 1,
		Size: 100,
	})
...
func TestGRPCProjectErrorHandling(t *testing.T) {
	h, _ := test.RegisterIntegration(t)
	h.StartControllersServer()
	...
	getReq := &pb.GetProjectRequest{Id: "nonexistent"}
	_, err = grpcClient.GetProject(context.Background(), getReq)
	Expect(err).To(HaveOccurred())

	deleteReq := &pb.DeleteProjectRequest{Id: "nonexistent"}
	_, err = grpcClient.DeleteProject(context.Background(), deleteReq)
}
