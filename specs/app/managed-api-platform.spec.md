...
The CLI is generated by CG-002 (`scripts/cli-generator/`) from the OpenAPI specification. It produces a standalone Cobra-based Go binary with `login`, `list`, `get`, `create` commands for each entity.
...
The CLI generator reads `openapi/openapi.yaml` (which `$ref`s the per-entity specs) and discovers all entities and their writable fields. It produces a complete `go.mod`-based project.

### Authentication

```bash
trex-cli login --token-file /dev/stdin --url http://localhost:8000 <<< "$OIDC_TOKEN"
trex-cli login --url http://localhost:8000  # no-auth mode (token optional)
```

The CLI stores the server URL and token in `~/.config/trex-cli/config.json` for subsequent calls.

### API-to-CLI Mapping

#### Projects

| REST API | `trex-cli` Command |
|---|---|
| `GET /projects` | `trex-cli list projects` |
| `GET /projects/{id}` | `trex-cli get project <id>` |
| `POST /projects` | `trex-cli create project --name <n> [--description <d>] [--repository-url <url>] [--status <s>]` |
| `PATCH /projects/{id}` | `trex-cli update project <id> [--name <n>] [--status <s>]` |
| `DELETE /projects/{id}` | `trex-cli delete project <id>` |

#### Entity Definitions

| REST API | `trex-cli` Command |
|---|---|
| `GET /entity_definitions` | `trex-cli list entity-definitions [--project-id <p>]` |
| `GET /entity_definitions/{id}` | `trex-cli get entity-definition <id>` |
| `POST /entity_definitions` | `trex-cli create entity-definition --project-id <p> --kind-name <k> [--plural-override <pl>] [--description <d>]` |
| `PATCH /entity_definitions/{id}` | `trex-cli update entity-definition <id> [--kind-name <k>] [--description <d>]` |
| `DELETE /entity_definitions/{id}` | `trex-cli delete entity-definition <id>` |

#### Field Definitions

| REST API | `trex-cli` Command |
|---|---|
| `GET /field_definitions` | `trex-cli list field-definitions [--entity-definition-id <e>]` |
| `GET /field_definitions/{id}` | `trex-cli get field-definition <id>` |
| `POST /field_definitions` | `trex-cli create field-definition --entity-definition-id <e> --field-name <n> --field-type <t> [--is-required]` |
| `PATCH /field_definitions/{id}` | `trex-cli update field-definition <id> [--field-name <n>] [--field-type <t>]` |
| `DELETE /field_definitions/{id}` | `trex-cli delete field-definition <id>` |

#### Relationships

| REST API | `trex-cli` Command |
|---|---|
| `GET /relationships` | `trex-cli list relationships [--project-id <p>]` |
| `GET /relationships/{id}` | `trex-cli get relationship <id>` |
| `POST /relationships` | `trex-cli create relationship --project-id <p> --source-entity-id <s> --target-entity-id <t> --relationship-type <type> [--foreign-key <fk>]` |
| `PATCH /relationships/{id}` | `trex-cli update relationship <id> [--relationship-type <type>]` |
| `DELETE /relationships/{id}` | `trex-cli delete relationship <id>` |

#### Builds

| REST API | `trex-cli` Command |
|---|---|
| `GET /builds` | `trex-cli list builds [--project-id <p>]` |
| `GET /builds/{id}` | `trex-cli get build <id>` |
| `POST /builds` | `trex-cli create build --project-id <p> [--triggered-by <user>]` |
| `DELETE /builds/{id}` | `trex-cli delete build <id>` |

### End-to-End CLI Workflow

```bash
# 1. Login
trex-cli login --url http://localhost:8000

# 2. Create a project
trex-cli create project --name my-service --description "My API service" --repository-url github.com/myorg/my-service

# 3. Activate the project
trex-cli update project <project-id> --status active

# 4. Define entities
trex-cli create entity-definition --project-id <project-id> --kind-name Customer
trex-cli create entity-definition --project-id <project-id> --kind-name Order

# 5. Define fields
trex-cli create field-definition --entity-definition-id <customer-id> --field-name email --field-type string --is-required
trex-cli create field-definition --entity-definition-id <customer-id> --field-name name --field-type string --is-required
trex-cli create field-definition --entity-definition-id <order-id> --field-name total --field-type float --is-required
trex-cli create field-definition --entity-definition-id <order-id> --field-name customer_id --field-type string --is-required

# 6. Define relationships
trex-cli create relationship --project-id <project-id> --source-entity-id <customer-id> --target-entity-id <order-id> --relationship-type has_many

# 7. Trigger a build
trex-cli create build --project-id <project-id> --triggered-by "developer@example.com"

# 8. Check build status
trex-cli get build <build-id>
```

---

## SDK Reference (CG-003)

The SDK is generated by CG-003 (`scripts/sdk-generator/`) from the OpenAPI specification. It produces typed client libraries in Go, Python, and TypeScript.
...
Standalone `go.mod`-based module. Provides typed client methods for all entities.
...
import (
    "context"
    "fmt"
    sdk "github.com/openshift-online/rh-trex-ai/sdk/go"
)

func main() {
    client := sdk.NewClient("http://localhost:8000", sdk.WithToken("..."))

    // Create a project
    project, err := client.Projects().Create(context.Background(), &sdk.Project{
        Name:        "my-service",
        Description: strPtr("My API service"),
        Status:      "draft",
    })

    // List entity definitions scoped to a project
    entityDefs, err := client.EntityDefinitions().List(context.Background(),
        sdk.WithProjectID(project.ID),
    )

    // Trigger a build
    build, err := client.Builds().Create(context.Background(), &sdk.Build{
        ProjectID: project.ID,
    })

    // Poll build status
    build, err = client.Builds().Get(context.Background(), build.ID)
    fmt.Println(build.Status) // pending, building, succeeded, or failed
}
```

### Python SDK

Pip-installable package with type hints.

```python
from trex_sdk import Client

client = Client("http://localhost:8000", token="...")

# Create a project
project = client.projects.create(name="my-service", description="My API")

# Define an entity
entity = client.entity_definitions.create(
    project_id=project.id,
    kind_name="Customer",
)

# Add fields
client.field_definitions.create(
...
# Trigger build
build = client.builds.create(project_id=project.id)
print(build.status)  # "pending"
```

### TypeScript SDK

Npm-publishable package with full TypeScript types. This is the foundation for the Console Plugin UI (CG-004) and any custom frontend.

```typescript
import { Client } from '@rh-trex/sdk';

const client = new Client('http://localhost:8000', { token: '...' });
...
const customer = await client.entityDefinitions.create({
  project_id: project.id,
  kind_name: 'Customer',
});

await client.fieldDefinitions.create({
  entity_definition_id: customer.id,
  field_name: 'email',
  field_type: 'string',
  is_required: true,
});
...
// Trigger build
const build = await client.builds.create({ project_id: project.id });
console.log(build.status); // "pending"
```

### SDK Coverage Matrix

| Entity | Go SDK | Python SDK | TypeScript SDK |
|---|---|---|---|
| Projects | `List`, `Get`, `Create`, `Update`, `Delete` | `list`, `get`, `create`, `update`, `delete` | `list`, `get`, `create`, `update`, `delete` |
| EntityDefinitions | `List`, `Get`, `Create`, `Update`, `Delete` | `list`, `get`, `create`, `update`, `delete` | `list`, `get`, `create`, `update`, `delete` |
| FieldDefinitions | `List`, `Get`, `Create`, `Update`, `Delete` | `list`, `get`, `create`, `update`, `delete` | `list`, `get`, `create`, `update`, `delete` |
| Relationships | `List`, `Get`, `Create`, `Update`, `Delete` | `list`, `get`, `create`, `update`, `delete` | `list`, `get`, `create`, `update`, `delete` |
| Builds | `List`, `Get`, `Create`, `Delete` | `list`, `get`, `create`, `delete` | `list`, `get`, `create`, `delete` |
...
| Area | API Server | Go SDK | Python SDK | TypeScript SDK | CLI (`trex-cli`) | Console UI | Notes |
|---|---|---|---|---|---|---|---|
| **Projects — CRUD** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | Status transitions, cascade delete |
| **EntityDefinitions — CRUD** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | Unique constraint on `(project_id, kind_name)` |
| **FieldDefinitions — CRUD** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | Unique constraint on `(entity_definition_id, field_name)` |
| **Relationships — CRUD** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | Same-project validation, self-reference rejection |
| **Builds — trigger/status** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | Subprocess-based generation, no PATCH |
| **Build controller — generation pipeline** | :white_check_mark: | n/a | n/a | n/a | n/a | n/a | Resolves ERD → generator arguments → subprocess |
| **Parent-child query filtering** | :white_check_mark: | 🔲 | 🔲 | 🔲 | 🔲 | 🔲 | `?project_id=`, `?entity_definition_id=` |
| **Generator flag mapping** | :white_check_mark: | n/a | n/a | n/a | n/a | n/a | EntityDefinition + FieldDefinition → `--kind --fields` |
| **CLI generation (CG-002)** | n/a | n/a | n/a | n/a | 🔲 | n/a | `scripts/cli-generator/` against merged OpenAPI spec |
| **SDK generation (CG-003)** | n/a | 🔲 | 🔲 | 🔲 | n/a | n/a | `scripts/sdk-generator/` — Go, Python, TypeScript |
| **Console UI (CG-004/APP-002)** | n/a | n/a | n/a | n/a | n/a | 🔲 | `scripts/console-plugin-generator/` or custom React app |
| **Library mode (`--library`)** | 🔲 | n/a | n/a | n/a | n/a | n/a | Downstream project generation |
